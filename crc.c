...
DWORD GetCrc32(const BYTE* pData, size_t nLength)//获取crc32值
{
    DWORD  fcs = 0xffffffff;
    size_t len = nLength;
    while(len--)
        fcs = (fcs >> 8) ^ crctab32[(fcs & 0xFF) ^ *pData++];
...
bool IsCrc16Good(const BYTE* pData, int nLength, WORD crcCode)
{
    ...
    fcs = (fcs >> 8) ^ crctab16[(fcs ^ (crcCode & 0xff)) & 0xff];
    fcs = (fcs >> 8) ^ crctab16[(fcs ^ ((crcCode >> 8) & 0xff) ) & 0xff];
